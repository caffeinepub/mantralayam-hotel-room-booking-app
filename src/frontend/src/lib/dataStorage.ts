...
const DEFAULT_HOTELS: Hotel[] = [
  {
    ...
    videoUrls: [],
    ratings: [5, 5, 4, 5, 5],
    ownerMessage: 'Welcome to Mantralayam Grand Hotel. We offer premium hospitality with modern amenities.',
    roomCount: 20,
    ownerType: 'admin',
  },
  {
...
const DEFAULT_HOMESTAYS: HomeStay[] = [
  {
    ...
    videoUrls: [],
    ratings: [5, 4, 5, 5, 4],
    ownerMessage: 'Welcome to our premium suite! We ensure a comfortable and memorable stay.',
    roomCount: 3,
    roomAvailability: [
      { roomIndex: 1, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 2, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 3, isAvailable: true, unavailableDateRanges: [], personCapacity: 3 },
    ],
  },
  {
    id: 'homestay-2',
    ...
    roomAvailability: [
      { roomIndex: 1, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 2, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 3, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 4, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 5, isAvailable: true, unavailableDateRanges: [], personCapacity: 3 },
    ],
  },
  {
    id: 'homestay-3',
    ...
    videoUrls: [],
    ratings: [5, 5, 4, 5, 5],
    ownerMessage: 'Perfect for families visiting the temple. Clean and spacious rooms with all amenities.',
    roomCount: 4,
    roomAvailability: [
      { roomIndex: 1, isAvailable: true, unavailableDateRanges: [], personCapacity: 4 },
      { roomIndex: 2, isAvailable: true, unavailableDateRanges: [], personCapacity: 4 },
      { roomIndex: 3, isAvailable: true, unavailableDateRanges: [], personCapacity: 5 },
      { roomIndex: 4, isAvailable: true, unavailableDateRanges: [], personCapacity: 4 },
    ],
  },
  {
    id: 'homestay-4',
    ...
    maxPrice: 4000,
    fixedPrice: 3500,
    amenities: ['King Bed', 'AC', 'WiFi', 'TV', 'Mini Bar', 'Work Desk', 'Balcony'],
    photos: [
    ...
    videoUrls: [],
    ratings: [5, 5, 5, 4, 5],
    ownerMessage: 'Luxury accommodation with premium facilities for discerning guests.',
    roomCount: 2,
    roomAvailability: [
      { roomIndex: 1, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 2, isAvailable: true, unavailableDateRanges: [], personCapacity: 3 },
    ],
  },
  {
    id: 'homestay-5',
    ...
    videoUrls: [],
    ratings: [4, 4, 4, 3, 4],
    ownerMessage: 'Affordable and clean accommodation for budget-conscious travelers.',
    roomCount: 8,
    roomAvailability: [
      { roomIndex: 1, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 2, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      ...
      { roomIndex: 4, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 5, isAvailable: true, unavailableDateRanges: [], personCapacity: 3 },
      { roomIndex: 6, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 7, isAvailable: true, unavailableDateRanges: [], personCapacity: 2 },
      { roomIndex: 8, isAvailable: true, unavailableDateRanges: [], personCapacity: 3 },
    ],
  },
];
...
const DEFAULT_TEMPLE_SPECIALS: TempleSpecials = {
  updates: [
    {
      id: 'update-1',
      imageLink: 'https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=1280&h=720&fit=crop',
      description: 'Experience the divine blessings at Sri Raghavendra Swamy Mutt. Special abhishekam and poojas are conducted throughout the day for devotees.',
      time: 'Morning Pooja at 7 AM',
      order: 1,
    },
    {
      id: 'update-2',
      imageLink: 'https://images.unsplash.com/photo-1548013146-72479768bada?w=1280&h=720&fit=crop',
      description: 'Join us for the evening aarti and experience the divine atmosphere. All devotees are welcome to participate in the sacred rituals.',
...
function resetStorage() {
  ...
  localStorage.setItem(STORAGE_KEYS.TEMPLE_SPECIALS, JSON.stringify(DEFAULT_TEMPLE_SPECIALS));

  console.log('Storage initialized with Schema v' + STORAGE_VERSION);
  console.log('Default hotels and homestays loaded with per-room availability and capacity');
}
...
export function initializeStorage() {
  try {
    ...
    if (currentVersion !== STORAGE_VERSION) {
      console.log(`Schema version change detected: ${currentVersion} -> ${STORAGE_VERSION}`);
      if (!validateSchema()) {
        resetStorage();
      } else {
      ...
      console.log('Storage validated successfully - Schema v' + STORAGE_VERSION);
...
function migrateToPerRoomAvailability() {
  try {
    ...
    const updated = homeStays.map(homeStay => {
      ...
      if (!homeStay.roomAvailability || homeStay.roomAvailability.length === 0) {
        migrated = true;
...
export function addNotification(message: string, type: 'booking' | 'payment' | 'partner' | 'customer' = 'booking') {
  try {
...
export function updateAnalytics(metric: keyof Analytics, increment: number = 1) {
  try {
...
export function getPartnerAuth(): { authenticated: boolean; partnerId?: string; partnerRoomId?: string } {
  try {
    const authenticated = localStorage.getItem('partnerAuthenticated') === 'true';
...
export function setPartnerAuth(authenticated: boolean, partnerId?: string, partnerRoomId?: string) {
  try {
    if (authenticated) {
...
export function saveTempleSpecials(specials: TempleSpecials) {
  try {
    localStorage.setItem(STORAGE_KEYS.TEMPLE_SPECIALS, JSON.stringify(specials));
...
export function getTotalCapacityForRooms(homeStayId: string, roomQuantity: number): number {
  const homeStays = getHomeStays();
  const homeStay = homeStays.find(h => h.id === homeStayId);
