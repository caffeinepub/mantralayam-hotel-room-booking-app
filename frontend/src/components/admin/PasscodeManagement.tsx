...
import { updateRoomPasscode, clearPartnerAuthCache, clearPartnerCachedData, getRooms } from '../../lib/roomStorage';
...
import { Loader2, Key, Trash2, Plus, AlertCircle, CheckCircle, Eye, EyeOff } from 'lucide-react';
...
export default function PasscodeManagement() {
  ...
  const handleAddPasscode = async () => {
    ...
    try {
      ...
      const duplicate = rooms.find(r => (r.passcode === newPasscode.trim() || r.password === newPasscode.trim()) && r.id !== newRoomId.trim());
      if (duplicate) {
        setError(`Passcode already in use for room: ${duplicate.name || duplicate.id}`);
        return;
      }
      ...
      setSuccess(`✅ Passcode set for room ${newRoomId.trim()}. Partner can now log in with this passcode.`);
      setNewRoomId('');
      setNewPasscode('');
      await loadPasscodes();
    } catch (err: unknown) {
      setError(`Failed to set passcode: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  const handleDeletePasscode = async (passcode: string) => {
    ...
    try {
      ...
      setError(`Failed to remove passcode: ${err instanceof Error ? err.message : String(err)}`);
    } finally {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xl font-bold text-foreground">Partner Passcode Management</h2>
      </div>

      <Card className="border border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/20">
        <CardContent className="p-4">
          <p className="text-sm text-amber-800 dark:text-amber-200">
            <strong>How it works:</strong> Set a passcode for a partner room here. The partner uses this passcode to log in to their dashboard. 
            Passcodes are stored in the backend and take effect immediately — no reload needed.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <Button onClick={handleAddPasscode} disabled={addingPasscode} className="gap-2">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <Badge variant="outline" className="text-xs font-mono">{config.roomId}</Badge>
                    </div>
                    <div>
