...
export function saveGoogleSheetsConfig(sheetUrl: string): void {
  try {
    ...
    window.dispatchEvent(new CustomEvent('googleSheetsConfigUpdated', { detail: config }));
  } catch (error) {
...
export function clearGoogleSheetsConfig(): void {
  ...
  window.dispatchEvent(new CustomEvent('googleSheetsConfigUpdated', { detail: null }));
}
