...
function isSessionError(err: unknown): boolean {
  ...
  return /unauthorized|session|expired|authentication|not authenticated/i.test(msg);
}

function handleMutationError(err: unknown, fallbackMsg = 'An error occurred. Please try again.') {
...
export function useAuthenticateAdmin() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Authentication failed. Check your credentials.'),
  });
}
...
export function useInitializeAccessControl() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to initialize access control.'),
  });
}
...
export function useUpdateHomeStay() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['homestays'] });
      queryClient.invalidateQueries({ queryKey: ['homestayDetails', variables.id] });
      queryClient.invalidateQueries({ queryKey: ['homestayDetails'] });
      queryClient.invalidateQueries({ queryKey: ['partnerRooms'] });
...
export function useCreateBooking() {
  ...
  return useMutation({
    ...
    onError: (err) => {
      if (isSessionError(err)) {
        toast.error('Your session has expired. Please log out and log back in to complete your booking.');
      } else {
        const msg = err instanceof Error ? err.message : String(err);
        if (/^unauthorized/i.test(msg)) {
          toast.error('Your session has expired. Please log out and log back in to complete your booking.');
...
export function useMarkNotificationAsRead() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to mark notification as read.'),
  });
}
...
export function useDeleteNotification() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to delete notification.'),
  });
}
...
export function useAuthenticatePartner() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Invalid password. Please try again.'),
  });
}
...
export function useAdminUpdatePartnerProfile() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to update partner profile.'),
  });
}
...
export function useUpdatePartnerRoomDetails() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['partnerRooms'] });
      queryClient.invalidateQueries({ queryKey: ['homestays'] });
      queryClient.invalidateQueries({ queryKey: ['homestayDetails', variables.roomId] });
      queryClient.invalidateQueries({ queryKey: ['homestayDetails'] });
...
export function useCreateGuestProfile() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to create guest profile.'),
  });
}
...
export function useUpdateGuestProfile() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to update guest profile.'),
  });
}
...
export function useCreateCheckoutSession() {
  ...
  return useMutation({
    ...
    onError: (err) => handleMutationError(err, 'Failed to create checkout session.'),
  });
}
...
export function useUpdateAdminCredentials() {
  ...
  return useMutation({
    mutationFn: async ({ newKey, newPassword }: { newKey: string; newPassword: string }) => {
      if (!actor) throw new Error('Actor not available');
